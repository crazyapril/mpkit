...
class Colormap:
    ...
    def cmap_listed(self):
        ...
        if not os.path.exists(filepath):
            filepath = os.path.join(os.path.dirname(os.path.abspath(
                sys.argv[0])), self.control['name']+'.cmap')
        with open(filepath, 'r') as f:
            ...
            for line in f:
                if line[0] == '*':
                    ...
                    if tval < last_tval:
                        raise ColormapDefinitionError('[Error 5] tval should be arranged in order. Line:'+line)
                    ...
                    if color == TRANSIT_COLOR:
                        ...
                        if transit_count > 0:
                            ...
                            for i in range(-transit_count, 0):
                                ...
                                color_list[i] = tuple((j - i) * ratio + i for i, j in zip(bcolor, color))
                            transit_count = 0
    ...
    def single_line_linear(self, line):
        ...
        lsplit = line.split(' ')
        if len(lsplit) != 3:
            raise ColormapDefinitionError('[Error 2] Only 2 spaces are allowed in a line. Line:'+line)
        try:
            tval = float(lsplit[0])
        except (SyntaxError, ValueError, NameError):
            raise ColormapDefinitionError('[Error 3] Invalid value. Line:'+line)
        if lsplit[1] == 'BEGIN':
            return tval, (0., 0., 0.), self.get_color(lsplit[2], line)
        elif lsplit[2] == 'END':
            return tval, self.get_color(lsplit[1], line), (0., 0., 0.)
        else:
            return tval, self.get_color(lsplit[1], line), self.get_color(lsplit[2], line)
    ...
    def single_line_listed(self, line):
        ...
        lsplit = line.split(' ')
        if len(lsplit) != 2:
            raise ColormapDefinitionError('[Error 2] Only 1 space are allowed in a line. Line:'+line)
        try:
            tval = float(lsplit[0])
        except (SyntaxError, ValueError, NameError):
            raise ColormapDefinitionError('[Error 3] Invalid value. Line:'+line)
    ...
    def get_color(self, l, line):
        ...
        b = l.split('/')
        if len(b) != 3:
            raise ColormapDefinitionError('[Error 4] Invalid color format. Line:'+line)
        for i in range(len(b)):
            ...
            v /= 255.0
            if v < 0 or v > 1:
                raise ColormapDefinitionError('[Error 6] Value out of range (0~255). Line:'+line)
            b[i] = v
        return tuple(b)
